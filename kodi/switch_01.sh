...
function play_random_video() {
    ...
    active=$(curl -s --header "Content-Type: application/json" \
      --data '{"jsonrpc":"2.0","method":"Player.GetActivePlayers","id":1}' \
      "$KODI_URL" | grep -q '"playerid":1' && echo "yes" || echo "no")

    if [ "$active" = "no" ]; then
      echo "->>> not active pl, open pl"
      # Open playlist ID 1
      curl -s --header "Content-Type: application/json" \
        --data '{"jsonrpc":"2.0","method":"Player.Open","params":{"item":{"playlistid":1}},"id":1}' \
        "$KODI_URL"
    else
      echo "->>> active pl, goto pl"
      curl -s --header "Content-Type: application/json" \
    ...
    # curl -s --header "Content-Type: application/json" --data '{"jsonrpc":"2.0","method":"Player.Open","params":{"item":{"playlistid":1}},"id":1}' http://localhost:8080/jsonrpc
    ...
    seek_percent=$(get_random_number 5 75)
